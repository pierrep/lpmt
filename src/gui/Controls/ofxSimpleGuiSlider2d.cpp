...
void ofxSimpleGuiSlider2d::draw(float x, float y) {
	...
	{
        ofDrawRectangle(0, 0, width, height - config->slider2DTextHeight);
	}
	ofFill();
	setTextBGColor();
	...
	{
        ofDrawRectangle(0, height-config->slider2DTextHeight, width, config->slider2DTextHeight);
	}
	...
	setTextColor();
    ofDrawLine(pointv.x-x, 0, pointv.x-x, height-config->slider2DTextHeight);
